...
{
    /**
     * ANDs the value in the accumulator (A) register with the value pointed at in memory,
    ...
    public class AND : IOperation
    {
        public static IDictionary<byte, IAddressingMode> OpcodeMap { get; } =
            ...
            {
                {0x20 + 0x01, IndexedIndirectX.Instance}, // Indexed indirect (d, x)
                {0x20 + 0x05, ZeroPage.Instance}, // Zero page d
                {0x20 + 0x09, Immediate.Instance}, // Immediate #i
                {0x20 + 0x0D, Absolute.Instance}, // Absolute a
                {0x20 + 0x11, IndirectIndexedY.Instance}, // Indirect Indexed (d), y
                {0x20 + 0x15, ZeroPageIndexedX.Instance}, // Zero page X d, x
                {0x20 + 0x19, AbsoluteIndexedY.Instance}, // Absolute, Y a, y
                {0x20 + 0x1D, AbsoluteIndexedX.Instance} // Absolute, X a, x
            };
